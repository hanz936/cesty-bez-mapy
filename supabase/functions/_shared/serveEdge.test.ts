...
Deno.test("composeEdge: OPTIONS gets 204+CORS without hitting withSupabase", async () => {
  const fetchFn = composeEdge({ auth: "publishable", fnName: "t" }, () => new Response("ok"), fakeWithSupabase);
...
Deno.test("composeEdge: authed request reaches handler with ctx", async () => {
  const fetchFn = composeEdge({ auth: "publishable", fnName: "t" }, (_r, ctx) => Response.json({ mode: (ctx as { authMode: string }).authMode }), fakeWithSupabase);

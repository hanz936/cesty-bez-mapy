...
- `supabase/_archive/migrations-pre-baseline/` — sem se přesune 46 původních migrací při konsolidaci (MIMO `migrations/`, aby je CLI nečetl)
...
```sql
-- neindexované foreign keys (heuristika: 1. sloupec FK == levý sloupec nějakého indexu)
-- Autoritativní zdroj je advisor 0001; tento dotaz je doplňkový cross-check.
select c.conrelid::regclass as table_name, c.conname as fk
...
Použij MCP `generate_typescript_types` (`project_id="dkblgznhnixubyoghrqe"`). Případně CLI (06/2026 flag tvar): `supabase gen types --linked --lang typescript` (typescript je default, positional `gen types typescript` je legacy).
...
Run: `supabase db dump --linked --keep-comments -f supabase/migrations/00000000000001_baseline.sql`
Expected: jeden soubor s plným schématem (RLS, granty, funkce, triggery, extensions, storage policies). `--keep-comments` je nutné — default pg_dump comments zahazuje a projekt je používá. Zvážit `-s public,storage`, pokud default schémata nezachytí storage policies.
...
- [ ] **Step 1: Přesunout 46 původních migrací do archivu MIMO `migrations/`**

```bash
# POZOR: archiv MUSÍ být mimo supabase/migrations/, aby ho CLI nečetl
mkdir -p supabase/_archive/migrations-pre-baseline
git mv supabase/migrations/0[0-4][0-9]_*.sql supabase/_archive/migrations-pre-baseline/ 2>/dev/null || true
git mv supabase/migrations/003_*.md supabase/_archive/migrations-pre-baseline/ 2>/dev/null || true
```
Pak ověřit, co v `migrations/` zůstalo: `ls supabase/migrations/`. **Cesta 1 (squash):** zkontrolovat skutečný stav po squashi — v `migrations/` má zůstat jen jeden baseline soubor; cokoliv jiného přesunout do archivu. Neasumovat automatické mazání, ověřit `ls`.
...
- [ ] **Step 4a: Verifikace — `db diff` nulový drift proti remote**

Run: `supabase db diff --linked`
Expected: **prázdný výstup**. Pokud něco hlásí → baseline je neúplná, NEzafixovat, doplnit (řeš přes `superpowers:systematic-debugging`).

- [ ] **Step 4b: Verifikace — introspekční rovnost (záchyt migra caveatů)**

`db diff` používá `migra`, který **prokazatelně nevidí** `alter policy`, grants, comments, security-invoker views (viz §8/§12 specu). Proto navíc porovnat introspekci mezi **lokální DB po `db reset`** a **remote**:
- Znovu spustit dotazy z Task 1.2 Step 2 (policies), Step 5 (grants execute), Step 4 (funkce + search_path) lokálně i na remote a **diffnout výstupy** (např. ulož obě a `diff`).
Expected: policies, granty, search_path, comments **identické** na obou stranách. Jakýkoliv rozdíl = baseline neúplná → doplnit ručně do baseline a opakovat 4a/4b.
...
**Ověření plánu proti dokům 06/2026 (Context7 `/supabase/cli` + Firecrawl):** flagy potvrzeny —
`db reset` má `--local` (default) i nebezpečné `--linked`; `db dump` je schema-only by default
(přidáno `--keep-comments`); `db diff --linked` diffuje lokální migrace proti linked projektu
(default migra → doplněn introspekční cross-check kvůli caveatům u policies/grantů/comments);
`migration squash --linked` squashne lokální soubory a srovná linked historii (mění jen
metadata, ne data); `gen types --linked --lang typescript` (legacy positional opraven).
Archiv migrací přesunut mimo `migrations/`. FK-index dotaz opraven na korektní PG syntaxi.

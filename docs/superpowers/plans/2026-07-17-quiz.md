...
### Task 0: FE — centrální Sentry breadcrumby v loggeru (prerekvizita T7)

**Files:**
- Modify: `src/utils/logger.ts`
- Test: `src/utils/logger.test.ts` (nový — logger dosud test nemá)

**Interfaces:**
- Consumes: `@sentry/react` (statický import; Sentry je už v main chunku přes `instrument.ts`, žádná změna bundle).
- Produces (spoléhá T7): `logger.warn(message)` a `logger.error(message, …)` nově VŽDY volají `Sentry.addBreadcrumb({ category: 'logger', level, message })`. Bez Sentry initu (dev/test/prerender — `enabled` v `instrument.ts`) je `addBreadcrumb` no-op (`if (!client) return` ve zdrojáku SDK 10.55.0), v produkci se breadcrumb odešle jen přiložený k případnému pozdějšímu error eventu. `info`/`debug` breadcrumb nepřidávají. Do breadcrumbu jde POUZE `message` (ne `data`) — PII disciplína.
...
- [ ] **Step 2: Ověř FAIL** — Run: `npx vitest run src/utils/logger.test.ts`
Expected: FAIL — `addBreadcrumb` se zatím nevolá (assertion error, ne import error).

- [ ] **Step 3: Napiš implementaci**

V `src/utils/logger.ts`:

1. Nahoru přidej import:

```ts
import * as Sentry from '@sentry/react'
```

2. V `error` nahraď zakomentovaný placeholder (`// Error monitoring service integration point` + řádek pod ním) a přidej breadcrumb za celý if/else blok, v `warn` nahraď komentář `// Production warnings are usually suppressed`:
...
(`info` a `debug` beze změny.)

- [ ] **Step 4: Ověř PASS + žádná regrese**

Run: `npx vitest run src/utils/logger.test.ts` → PASS (3 testy).
Run: `npx vitest run` → celá sada zelená (logger je mockovaný ve všech testech, které ho používají; reálný logger nikde breadcrumb neodešle — Sentry se v testech neinituje).

- [ ] **Step 5: Commit**

```bash
git add src/utils/logger.ts src/utils/logger.test.ts
git commit -m "feat(logger): emit sentry breadcrumbs from warn and error"
```
...
- Produces (používá Task 8):
  ...
  - Vrací jen produkty s validním `quiz_data` a `enabled: true`; nevalidní přeskočí s `logger.warn` — Sentry breadcrumb vzniká centrálně v loggeru (Task 0), hook Sentry přímo NEVOLÁ (vznikla by duplicitní stopa).
...
- **Sentry breadcrumby vznikají centrálně v loggeru (Task 0):** feature kód `addBreadcrumb` přímo nevolá (duplicitní stopa); `Sentry.captureException` pro skutečné chyby aplikace zůstává per-callsite (vzor reviews komponent).

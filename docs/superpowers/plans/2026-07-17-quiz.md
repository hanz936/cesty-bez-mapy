...
const SealBadge = ({ variant, tier, score, size = 'lg', className = '' }: SealBadgeProps) => (
  <svg
    viewBox="0 0 120 120"
    aria-hidden="true"
    className={`rotate-[10deg] drop-shadow-[0_4px_10px_rgba(0,0,0,0.35)] motion-reduce:transform-none ${className}`.trim()}
  >
...
describe('QuizQuestion', () => {
  it('vykreslí nadpis, radiogroup se 4 přístupnými radii a progressbar s aria-valuetext', () => {
    renderQuestion({ index: 2 });
    expect(screen.getByRole('heading', { name: QUIZ_QUESTIONS[0].question })).toBeInTheDocument();
  ...
  it('„Další" je disabled bez výběru, enabled s výběrem; poslední otázka má „Vyhodnotit"', () => {
    renderQuestion();
    expect(screen.getByRole('button', { name: 'Další →' })).toBeDisabled();

    renderQuestion({ selectedIndex: 1 });
    expect(screen.getAllByRole('button', { name: 'Další →' }).at(-1)).toBeEnabled();

    renderQuestion({ index: 8, selectedIndex: 0 });
    expect(screen.getByRole('button', { name: 'Vyhodnotit' })).toBeEnabled();
  });
    ...
    <div>
      ...
      <div role="radiogroup" aria-labelledby={headingId} className="grid grid-cols-2 gap-4 sm:gap-5 md:grid-cols-4">
        {question.options.map((option, i) => {
          ...
          return (
            <label
              key={option.text}
              className={`relative cursor-pointer rounded bg-[#fbf9f3] p-1.5 pb-2 text-center shadow-[0_10px_26px_rgba(0,0,0,0.42)] transition-transform duration-200 focus-within:ring-2 focus-within:ring-green-500 focus-within:ring-offset-2 motion-reduce:transform-none motion-reduce:transition-none ${
                checked
      ...
      <div className="mt-7 flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-lg border-2 border-white/35 bg-green-800 px-5 py-2.5 text-sm font-bold text-white shadow-lg disabled:cursor-not-allowed disabled:opacity-50"
        >
...
const PostcardPhoto = ({ product, aspect }: { product: QuizProduct; aspect: string }) =>
  product.image_url ? (
    <img src={product.image_url} alt="" className={`${aspect} w-full object-cover shadow-[inset_0_0_0_1px_rgba(0,0,0,0.12)]`} />
  ) : (
...
const CustomItineraryNote = ({ prominent = false, onCustomClick }: { prominent?: boolean; onCustomClick: () => void }) => (
  <div
    className={`mx-auto mt-7 max-w-md rotate-[0.8deg] rounded border-t-[14px] border-green-800/15 bg-[#fffef7] p-5 shadow-[0_10px_26px_rgba(0,0,0,0.4)] motion-reduce:transform-none ${
    ...
    <div>
      ...
      {winner === undefined ? (
        ...
        <>
          ...
          <div className="relative mx-auto mt-6 max-w-xl -rotate-[1.3deg] rounded-md bg-[#fbf9f3] p-3 pb-4 shadow-[0_14px_34px_rgba(0,0,0,0.45)] motion-reduce:transform-none">
            ...
            <div className="mx-auto mt-6 grid max-w-xl gap-4 sm:grid-cols-2">
              {alternatives.map((match, i) => (
                <div
                  ...
                  className={`relative rounded-md bg-[#fbf9f3] p-2 pb-3 shadow-[0_14px_34px_rgba(0,0,0,0.45)] motion-reduce:transform-none ${
...
- Produces (používá Task 8):
  ...
  - Vrací jen produkty s validním `quiz_data` a `enabled: true`; nevalidní přeskočí s `logger.warn`.
...
describe('Quiz page', () => {
  ...
  it('rozpracované odpovědi přežijí refresh — start pokračuje první nezodpovězenou', () => {
    ...
    renderQuiz();
    fireEvent.click(screen.getByRole('button', { name: 'Začít kvíz' }));
    expect(screen.getByRole('heading', { name: QUIZ_QUESTIONS[2].question })).toBeInTheDocument();
  });
...
- Produces (používá Task 11):
  ...
  - `GATING_DIMENSIONS: readonly QuizDimension[]` (= `['season','duration']`)
  - `normalizeQuizData(data: Record<string, unknown>): Record<string, unknown>` — transform pro Create/Edit (doplní `version: 1`, `enabled ?? false`, nuly za nevyplněné klíče)
  - `validateQuizGating(values: Record<string, unknown>): object` — form-level validace (vnořený errors objekt react-hook-form tvaru)
  - `interface QuizProfileWarning { dimension: QuizDimension; message: string }`
  - `analyzeQuizProfile(profile: Record<string, Record<string, number | undefined>> | undefined): QuizProfileWarning[]`
...
describe("validateQuizGating (spec §8)", () => {
  it("vypnutý kvíz → žádné chyby", () => {
    expect(validateQuizGating({ quiz_data: { enabled: false, profile: fullProfile(0) } })).toEqual({});
  });

  it("zapnutý kvíz + celá vylučovací dimenze na nule → chyba na první klíč dimenze", () => {
    const errors = validateQuizGating({
      quiz_data: { enabled: true, profile: fullProfile(2, { season: { spring: 0, summer: 0, autumn: 0, winter: 0 } }) },
    }) as { quiz_data?: { profile?: { season?: { spring?: string } } } };
    expect(errors.quiz_data?.profile?.season?.spring).toMatch(/aspoň jednu hodnotu/);
  });

  it("zapnutý kvíz s aspoň jednou kladnou hodnotou v season i duration → OK", () => {
    expect(validateQuizGating({ quiz_data: { enabled: true, profile: fullProfile(1) } })).toEqual({});
  });
});
...
interface QuizFormData {
  enabled?: boolean;
  profile?: Record<string, Record<string, number | undefined>>;
}
...
export function normalizeQuizData(data: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [dimension, keys] of Object.entries(QUIZ_DIMENSIONS)) {
    profile[dimension] = {};
    for (const key of keys) {
      profile[dimension][key] = quiz.profile?.[dimension]?.[key] ?? 0;
    }
  }
...
/**
 * Form-level validace (react-hook-form vnořený errors tvar): při zapnutém kvízu
 * musí mít vylučovací dimenze aspoň jednu hodnotu > 0 (spec §8).
 */
export function validateQuizGating(values: Record<string, unknown>): object {
  const quiz = values.quiz_data as QuizFormData | undefined;
  if (!quiz?.enabled) return {};
  const profileErrors: Record<string, Record<string, string>> = {};
  for (const dimension of GATING_DIMENSIONS) {
    const keys = QUIZ_DIMENSIONS[dimension];
    const hasPositive = keys.some((key) => (quiz.profile?.[dimension]?.[key] ?? 0) > 0);
    if (!hasPositive) {
      profileErrors[dimension] = {
        [keys[0]]:
          "Vylučovací otázka musí mít aspoň jednu hodnotu vyšší než 0 — jinak se produkt v kvízu nikdy nenabídne.",
      };
    }
  }
  return Object.keys(profileErrors).length > 0 ? { quiz_data: { profile: profileErrors } } : {};
}
...
export function analyzeQuizProfile(
  profile: Record<string, Record<string, number | undefined>> | undefined,
): QuizProfileWarning[] {
  ...
  for (const [dimension, keys] of Object.entries(QUIZ_DIMENSIONS) as Array<[QuizDimension, readonly string[]]>) {
    const values = keys.map((key) => profile?.[dimension]?.[key] ?? 0);
...
- [ ] **Step 4: Ověř PASS** — Run: `npx vitest run src/resources/products/quizProfile.test.ts` → PASS (9 testů).
...
**Interfaces:**
- Consumes: Task 10 (`QUIZ_DIMENSIONS`, `QUIZ_ADMIN_LABELS`, `GATING_DIMENSIONS`, `analyzeQuizProfile`, `normalizeQuizData`, `validateQuizGating`).
- Produces: `ProductQuizTab` (named export) — obsah tabu; zapojení `transform` + `validate` do obou formulářů.
  ...
  <>
    ...
    {(Object.keys(QUIZ_DIMENSIONS) as QuizDimension[]).map((dimension) => (
      <Box key={dimension} sx={{ mb: 3 }}>
        ...
        {QUIZ_DIMENSIONS[dimension].map((key) => (
...
import { normalizeQuizData, validateQuizGating } from "./quizProfile";
```

2. `<Create>` → `<Create transform={normalizeQuizData}>`
3. `<TabbedForm warnWhenUnsavedChanges>` → `<TabbedForm warnWhenUnsavedChanges validate={validateQuizGating}>`
4. Před uzavírací `</TabbedForm>` přidej poslední tab:
...
`src/resources/products/ProductEdit.tsx` — stejné čtyři úpravy: importy, `<Edit transform={normalizeQuizData}>` (ostatní props `<Edit>` nech beze změny), `validate={validateQuizGating}` na `<TabbedForm>`, tab „Kvíz" jako poslední před `</TabbedForm>`.

- [ ] **Step 4: Ověř type-check, lint, testy, build**
...
- zapni kvíz, nech termín na nulách → uložení zablokuje chyba na záložce Kvíz;
- vyplň vše ≥ 2 v jedné dimenzi → objeví se varování „nic nerozliší";
- ulož validní profil a zkontroluj v DB (`quiz_data.version === 1`, doplněné nuly).
